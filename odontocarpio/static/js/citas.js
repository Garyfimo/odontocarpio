...
function fillHourByDayAndDoctor(doctor, day) {
    ...
    if (doctor === "ruesta") {
        ...
        if (day === "Miercoles" || day === "Viernes" || day === "Sábado") {
            hour = "<option>9:00</option>" +
                    "<option>10:00</option>" +
                    "<option>11:00</option>" +
                    "<option>12:00</option>" +
...
function solicitarCita() {
    $.ajax({
        ...
        data: {from: $("#e-mail").val(),
            ...
            message: "[CORREO AUTOGENERADO DESDE LA WEB]\n" + $("#name").val() + " ha solicitado una cita con " + $("#doctor option:selected").text() + " el dia " + $("#day option:selected").text() + " a las " + $("#hour option:selected").text() +" horas por el motivo " + $("#message").val() + " comunicarse a "+ $("#telephone").val() + " para confirmar la cita."
        },
        dataType: 'json',
        beforeSend: function() {
